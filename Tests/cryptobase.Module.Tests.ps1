...
$classes = [IO.File]::ReadAllLines([IO.Path]::Combine($BuildOutDir.FullName, "cryptobase.psm1")).Where({ $_.StartsWith("class") -or $_.StartsWith("enum ") }).ForEach({ $_.Replace("class ", '').Replace("enum ", '') }).ForEach({ ($_ -like "* : *") ? $_.split(" : ")[0] + '' : $_.Replace(' {', '') })

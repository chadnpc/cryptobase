$MODULE_DATA = PsModuleBase\Read-ModuleData -File .\en-US\cryptobase.strings.psd1
...
$classes = [IO.File]::ReadAllLines([IO.Path]::Combine($ModulePath, "cryptobase.psm1")).Where({ $_.StartsWith("class") -or $_.StartsWith("enum ") }).ForEach({ $_.Replace("class ", '').Replace("enum ", '') }).ForEach({ ($_ -like "* : *") ? $_.split(" : ")[0] + '' : $_.Replace(' {', '') })
